...
function Convert-Scalar {
    ...
    if ([double]::TryParse($trimmed, [System.Globalization.NumberStyles]::Float, [System.Globalization.CultureInfo]::InvariantCulture, [ref]$decimalNumber)) {
...
function Parse-SimpleYaml {
    ...
    foreach ($rawLine in [System.IO.File]::ReadAllLines($Path)) {
        ...
        if ([string]::IsNullOrWhiteSpace($rawValue)) {
            ...
            $parentFrame.Value[$key] = Convert-Scalar -Value $rawValue
        }
    }
    return $root
}
...
function Get-ConfigValue {
    param([hashtable]$Config, [string[]]$Path, [switch]$Optional)
...
function Assert-ConfigSchema {
    ...
    $kind = [string](Get-ConfigValue -Config $Config -Path @('platform', 'kind'))
    $target = [string](Get-ConfigValue -Config $Config -Path @('platform', 'target'))
    if ($kind -notin @('mobile', 'web', 'desktop')) {
        throw 'platform.kind must be mobile, web, or desktop'
    }
    $validCombination = (
    ...
    if (-not $validCombination) {
        throw "Unsupported platform combination: $kind/$target"
    }
...
function Get-ModuleToken {
    ...
    if ($Module -notmatch '^:?([A-Za-z0-9_-]+(?::[A-Za-z0-9_-]+)*)$') {
        throw "Unsupported module name: $Module"
    }
...
function Test-ModuleExists {
    ...
    foreach ($settingsFile in $settings) {
        ...
        foreach ($moduleMatch in [regex]::Matches($content, '["''](:[A-Za-z0-9_:-]+)["'']')) {
...
function Test-VariantExists {
    ...
    if (Test-Path -LiteralPath (Join-Path $modulePath "src/$Variant") -PathType Container) { return $true }
    $buildFiles = @(
        (Join-Path $modulePath 'build.gradle'),
        (Join-Path $modulePath 'build.gradle.kts')
    ) | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf }
    foreach ($buildFile in $buildFiles) {
        if ([System.IO.File]::ReadAllText($buildFile) -match "\b$([regex]::Escape($Variant))\b") { return $true }
    }
    return $false
}
...
function Get-SafeGradleTask {
    param([object]$Value)
    $task = [string]$Value
    if ([string]::IsNullOrWhiteSpace($task) -or $task -notmatch '^:[A-Za-z0-9_-]+(?::[A-Za-z0-9_-]+)+$') {
        throw "Unsupported Gradle task: $task"
    }
    return $task
}
...
$configAllowedPaths = @(
    ...
    'launch', 'launch.mode', 'launch.target', 'launch.activity', 'launch.deeplink', 'launch.task',
    'launch.url', 'launch.window_title', 'launch.entry_state',
...
$kind = [string](Get-ConfigValue -Config $config -Path @('platform', 'kind'))
$platformTarget = [string](Get-ConfigValue -Config $config -Path @('platform', 'target'))
...
if ($kind -eq 'mobile' -and $platformTarget -eq 'android') {
    ...
    $assembleTask = "$moduleToken`:assemble$variantName"
    $installTask = "$moduleToken`:install$variantName"
    Write-Output "ASSEMBLE: $wrapperPath $assembleTask"
    Write-Output "INSTALL: $wrapperPath $installTask"
    Write-Output ("LAUNCH: adb " + ($launchArguments -join ' '))
    if ($DryRun) { Write-Output 'DRY-RUN'; return }

    & $wrapperPath $assembleTask
    if ($LASTEXITCODE -ne 0) { throw "Gradle assemble command failed with exit code $LASTEXITCODE" }
    & $wrapperPath $installTask
...
$task = Get-SafeGradleTask -Value (Get-ConfigValue -Config $config -Path @('launch', 'task'))
if ($kind -eq 'web') {
    $url = [string](Get-ConfigValue -Config $config -Path @('launch', 'url'))
    if ([string]::IsNullOrWhiteSpace($url)) { throw 'Missing required config value: launch.url' }
    Write-Output "URL: $url"
} elseif ($kind -eq 'desktop') {
    $windowTitle = [string](Get-ConfigValue -Config $config -Path @('launch', 'window_title'))
    if ([string]::IsNullOrWhiteSpace($windowTitle)) { throw 'Missing required config value: launch.window_title' }
    Write-Output "WINDOW: $windowTitle"
} elseif ($kind -eq 'mobile' -and $platformTarget -eq 'ios') {
    Write-Output 'TARGET: iOS project-native simulator or device workflow'
}

Write-Output "RUN: $wrapperPath $task"
if ($DryRun) { Write-Output 'DRY-RUN'; return }

& $wrapperPath $task
if ($LASTEXITCODE -ne 0) { throw "Gradle run command failed with exit code $LASTEXITCODE" }
